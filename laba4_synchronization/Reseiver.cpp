...
void CloseAllHandlesR(HANDLE& hSemaphoreForReceiver,
    HANDLE& hSemaphoreForSender, HANDLE& hSemaphoreForConcoles)
{
...
int main() {
    ...
    fout.close();
    HANDLE hSemaphoreForReceiver, hSemaphoreForSender, hSemaphoreForConcoles;
    ...
    for (int i = 0; i < n; i++) {
        wsprintfA(for_senders, "laba4.exe %s %d %d", bin_file, i, size);
    ...
    while (true)
    {
        ...
        if (choice == 'r' || choice == 'R')
        {
            ...
            if (!size_messages)
            {
                ...
                {
                    CloseAllHandlesR( hSemaphoreForReceiver,
                        hSemaphoreForSender, hSemaphoreForConcoles);
                    CloseHandle(*hEvent);
